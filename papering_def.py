...
def openUrl(subject, year, season, paper):
    ...
    que_url = f"https://papers.gceguide.com/A%20Levels/{subject}/" \
        f"{str(year)}/{subject[-5:-1]}_{season}{str(year[2:])}_qp_{paper}.pdf"

    ans_url = f"https://papers.gceguide.com/A%20Levels/{subject}/" \
        f"{str(year)}/{subject[-5:-1]}_{season}{str(year[2:])}_ms_{paper}.pdf"
